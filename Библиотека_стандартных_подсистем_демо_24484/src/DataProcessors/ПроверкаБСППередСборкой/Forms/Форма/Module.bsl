...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьПроверку(НСтр("ru = 'Недопустимо использовать группы ""Горизонтальная если возможно""'"),
		"Подключаемый_ПроверкаИспользованияСвойстваГоризонтальнаяЕслиВозможно()");
...
Функция Подключаемый_ПроверитьКорректностьКомментированияДемоПримеров()
	...
	Для Каждого ФайлМодуля Из МассивМодулей Цикл
		...
		Если СтрНайти(ФайлМодуля.Имя, "ПереходНаВерсию231") > 0
...
Функция Подключаемый_НайтиНерекомендуемыеНастройкиРолей()
	...
	Если ЗначениеЗаполнено(Результат.НекорректнаяНастройкаПрав) Тогда
		СписокОшибочныхОбъектов.Добавить(Результат.НекорректнаяНастройкаПрав);
	КонецЕсли;
	ПредставлениеОшибки =
		НСтр("ru = 'Настройки видимости, просмотра и редактирования по ролям должны быть ПриСозданииНаСервере.'");
	...
	ПредставлениеОшибки =
		НСтр("ru = 'Права на реквизиты должны быть как значения по умолчанию, если сняты права на объект.'");
...
Функция РолиИсключения()
	...
	СтруктураИсключений.ЗапретДоступаКДругимПодсистемамБСП.Добавить("БазовыеПрава");
	СтруктураИсключений.ЗапретДоступаКДругимПодсистемамБСП.Добавить("БазовыеПраваВнешнегоПользователя");
