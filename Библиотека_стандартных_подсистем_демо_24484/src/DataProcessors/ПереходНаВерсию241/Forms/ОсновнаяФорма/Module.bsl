...
Процедура КаталогВыгрузкиКонфигурацииВФайлыОткрытие(Элемент, СтандартнаяОбработка)
	...
	#Если НЕ ВебКлиент Тогда
		...
		Тип = СистемнаяИнформация.ТипПлатформы;
		Если Тип = ТипПлатформы.Windows_x86 ИЛИ Тип = ТипПлатформы.Windows_x86_64 Тогда
			ЗапуститьПриложение("explorer.exe /select, " + Каталог);
		ИначеЕсли Тип = ТипПлатформы.Linux_x86 ИЛИ Тип = ТипПлатформы.Linux_x86_64 Тогда
			КомандаСистемы("nautilus "+ Каталог);
		КонецЕсли;
	#КонецЕсли
КонецПроцедуры
