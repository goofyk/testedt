...
Функция ВыгрузитьКонфигурациюВXML(Параметры)
	...
	Если ОткрытКонфигуратор() Тогда
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			ПутьККонфигурации = "File=" + Кавычка + КаталогКопии1CD + Кавычка + ";";
		Иначе
	...
	ЗапуститьПриложение(Кавычка + КаталогBIN + "1cv8.exe" + Кавычка + " DESIGNER"
		...
		+ " /DumpConfigToFiles "  + Кавычка + Параметры.РабочийКаталог + Кавычка
...
Функция ЗагрузитьКонфигурациюИзXML(Параметры)
	...
	СтрокаЗапуска = Кавычка + КаталогBIN + "1cv8.exe" + Кавычка + " DESIGNER"
		+ " /IBConnectionString "  + Кавычка + ПутьККонфигурации + Кавычка
		+ ?(Параметры.Пользователь = "",
			"",
			" /N " + Кавычка + Параметры.Пользователь + Кавычка + " /P " + Кавычка + Параметры.Пароль + Кавычка)
		+ " /LoadConfigFromFiles " + Кавычка + Параметры.РабочийКаталог + Кавычка
		+ " -listfile "            + Кавычка + ИмяФайлаИзмененныеФайлы + Кавычка
		+ " /Out "                 + Кавычка + ИмяФайлаСообщений + Кавычка
		+ " /DisableStartupDialogs"   // Без стартового диалога и диалогов аутентификации.
		+ " /DisableStartupMessages"; // Без стартовых предупреждений.
	...
	ЗаписьТекста.Записать(СтрокаЗапуска);
	ЗаписьТекста.Закрыть();
