...
Процедура Зарегистрироваться(Команда)
	...
	Если СостояниеРегистрации = "НеЗарегистрирована" Тогда 
		ПриПолученииКодаРегистрации(СерверВзаимодействия, АдресЭлектроннойПочты);
	ИначеЕсли СостояниеРегистрации = "ОжиданиеВводаКодаПодтверждения" Тогда 
		ПриРегистрации(СерверВзаимодействия, АдресЭлектроннойПочты, ИмяИнформационнойБазы, КодРегистрации);
	КонецЕсли;
...
&НаКлиенте
Процедура ПриПолученииКодаРегистрации(СерверВзаимодействия, АдресЭлектроннойПочты)
...
&НаКлиенте
Процедура ПриРегистрации(СерверВзаимодействия, АдресЭлектроннойПочты, ИмяИнформационнойБазы, КодРегистрации)
...
Процедура ПослеУспешнойРегистрации(РегистрацияВыполнена, ТекстСообщения, Контекст) Экспорт
	...
	Если РегистрацияВыполнена Тогда 
		...
		КонтейнерСостояний.СостояниеРегистрации = ТекущееСостояниеРегистрации();
	Иначе 
		ПоказатьПредупреждение(, ТекстСообщения);
		КонтейнерСостояний.СостояниеРегистрации = "ОжиданиеВводаКодаПодтверждения";
	КонецЕсли;
