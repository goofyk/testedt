...
Функция ПараметрыПолученияФайла() Экспорт
	...
	ПараметрыПолучения.Вставить("Таймаут", -1); // Автоматическое определение таймаута по размеру файла.
...
Функция ПолучитьФайлИзИнтернет(Знач URL, Знач НастройкаСохранения, Знач НастройкаСоединения,
	...
	Если ИспользуетсяFTPПротокол Тогда
		Попытка
			...
			Если Таймаут < 0 Или Таймаут = Неопределено Тогда
				Соединение = Новый FTPСоединение(ИмяСервера, Порт, ИмяПользователя, ПарольПользователя,
					Прокси, ПассивноеСоединение, 10, ЗащищенноеСоединение, УровеньИспользованияЗащищенногоСоединения);
				Таймаут = ОпределитьТаймаут(Соединение, ПутьКФайлуНаСервере);
			КонецЕсли;
		...
		Исключение
			...
			СообщениеОбОшибке = НСтр("ru = 'Не удалось установить FTP-соединение с сервером %1:'")
				+ Символы.ПС + "%2";
			
			ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке, 
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке + Символы.ПС + "%3", 
				ИмяСервера, КраткоеПредставлениеОшибки(ИнформацияОбОшибке), Результат.ОписаниеОшибки);
	...
	Иначе
		...
		Попытка
			...
			Если Таймаут < 0 Или Таймаут = Неопределено Тогда
				Соединение = Новый HTTPСоединение(ИмяСервера, Порт, ИмяПользователя, ПарольПользователя,
					Прокси, 10, ЗащищенноеСоединение, ИспользоватьАутентификациюОС);
				Таймаут = ОпределитьТаймаут(Соединение, ПутьКФайлуНаСервере, Заголовки);
			КонецЕсли;
			
			Соединение = Новый HTTPСоединение(ИмяСервера, Порт, ИмяПользователя, ПарольПользователя,
					Прокси, Таймаут, ЗащищенноеСоединение, ИспользоватьАутентификациюОС);
		...
		Если ЗаписыватьОшибку Тогда
			ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке, 
					URL, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
		КонецЕсли;
		
		Результат = ОбщегоНазначенияКлиентСервер.ДиагностикаСоединения(URL);
		
		Возврат СформироватьРезультат(Ложь, КраткоеПредставлениеОшибки(ИнформацияОбОшибке)
			+ Символы.ПС + Результат.ОписаниеОшибки, HTTPОтвет);
	КонецПопытки;
...
Функция ОпределитьТаймаут(Соединение, Знач ПутьКФайлуНаСервере, Знач Заголовки = Неопределено)
	...
	Если ТипЗнч(Соединение) = тип("FTPСоединение") Тогда
		...
		НайденныеФайлы = Соединение.НайтиФайлы(ПутьКФайлуНаСервере);
		Если НайденныеФайлы.Количество() > 0 Тогда
			Размер = НайденныеФайлы[0].Размер();
		КонецЕсли;
	...
	Иначе
		...
		HTTPЗапрос = Новый HTTPЗапрос(ПутьКФайлуНаСервере, Заголовки);
		ПолученныеЗаголовки = Соединение.ПолучитьЗаголовки(HTTPЗапрос);
		РазмерСтрокой = ПолученныеЗаголовки.Заголовки["Content-Length"];
		Размер = ТипЧисло.ПривестиЗначение(РазмерСтрокой);
