...
//
//       * РазрешеноВыбиратьИНастраиватьВариантыБезСохранения - Булево - Имеет смысл для тех отчетов,
//           у которых есть необходимость выбора и настройки предопределенных вариантов. Если Истина,
//           то блокируется кнопка сохранения варианта этого отчета,  но остается возможность выбора и
//           настройки предопределенных вариантов. Актуально для контекстных отчетов при необходимости
//           предоставлять выбор вариантов. Следует понимать, что при установленном флажке изменить вариант
//           возможность есть, но сохранить изменения не получится - при закрытии формы отчета или смене
//           варианта настройки будут утеряны.
//
...
Функция ПараметрыКопирования(ТипЭлемента, Коллекция)
	...
	Результат.Вставить("КопируетсяДеревоЗначений", Ложь);
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ИначеЕсли ТипЭлемента = Тип("СтрокаДереваЗначений") Тогда
		
		Результат.КопируетсяДеревоЗначений = Истина;
...
Функция ЗаполнитьСвойстваРекурсивно(Узел, ЧтоЗаполнить, ЧемЗаполнить, Соответствие = Неопределено, ПараметрыКопирования = Неопределено) Экспорт
	...
	Если ПараметрыКопирования.КопируетсяДеревоФормы
		Или ПараметрыКопирования.КопируетсяДеревоЗначений Тогда
		
		Соответствие.Вставить(ЧемЗаполнить, ЧтоЗаполнить);
		
		КоллекцияВложенных = ?(ПараметрыКопирования.КопируетсяДеревоФормы,
			ЧемЗаполнить.ПолучитьЭлементы(),
			ЧемЗаполнить.Строки);
		Если КоллекцияВложенных.Количество() > 0 Тогда
			НоваяКоллекцияВложенных = ?(ПараметрыКопирования.КопируетсяДеревоФормы,
				ЧтоЗаполнить.ПолучитьЭлементы(),
				ЧтоЗаполнить.Строки);
