...
Функция ВыполнитьЗапрос(ИмяМетода, ПараметрыЗапроса)
	...
	Попытка
		...
		Соединение = Новый HTTPСоединение("sms4b.ru", , , , ПолучениеФайловИзИнтернетаКлиентСервер.ПолучитьПрокси("https"), 60, Новый ЗащищенноеСоединениеOpenSSL(, Сертификаты));
		Соединение.ОтправитьДляОбработки(ИмяФайлаЗапроса, "/ws/s1c.asmx/" + ИмяМетода, ИмяФайлаОтвета, Заголовок);
