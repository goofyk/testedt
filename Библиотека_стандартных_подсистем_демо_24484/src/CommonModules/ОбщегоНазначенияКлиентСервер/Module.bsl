...
#КонецЕсли
	...
	Если Null = ПредопределенноеЗначение Тогда 
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		ВызватьИсключение НСтр("ru = 'Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
		                             |Повешение привилегий системы не доступно на сервере.'");
#КонецЕсли
		
		Если ДождатьсяЗавершения Тогда
			ВызватьИсключение НСтр("ru = 'Недопустимо одновременно устанавливать параметры 
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ОписаниеОшибки = ОписаниеОшибки + Символы.ПС 
			+ НСтр("ru = 'Подключение проводится на сервере 1С:Предприятия в интернете.'");
	Иначе 
...
#Иначе 
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Подключение проводится на клиентском компьютере <%1>.'"), ИмяКомпьютера());
#КонецЕсли
...
Функция ЖурналНастроекСистемногоПрокси()
	...
	Иначе 
		ЖурналПрокси = ЖурналПрокси 
			+ НСтр("ru = 'Параметры прокси-сервера: Системный настройки прокси-сервера не заданы.'");
	КонецЕсли;
...
Функция ПроверитьДоступностьСервера(АдресСервера)
	...
	СтрокаКоманды = СтрШаблон(ШаблонКоманды, АдресСервера);
	...
	Если ЭтоWindows Тогда
		Доступен = (СтрНайти(ЖурналДиагностики, "(0% потерь)") > 0)
			Или (СтрНайти(ЖурналДиагностики, "(0% less)") > 0);
	Иначе 
		Доступен = (СтрНайти(ЖурналДиагностики, "0% packet loss") > 0);
	КонецЕсли;
