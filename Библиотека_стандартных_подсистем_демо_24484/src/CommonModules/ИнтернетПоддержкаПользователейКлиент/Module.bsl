...
// Интернет-поддержки.
// При успешном завершении возвращаются введенные пользователем данные
// аутентификации через объект ОписаниеОповещения.
//
// Параметры:
...
Процедура ОтправитьСообщениеВТехПоддержку(
	...
	ЭтоФайловаяИБ = (ВРег(Лев(СтрокаСоединенияИнформационнойБазы(), 5)) = "FILE=");
	ПараметрыСообщения.Вставить("ОтправкаНаСервере", Не ЭтоФайловаяИБ);
	
	#Если ВебКлиент Тогда
	ПодготовитьВложенияКОтправкеНаСервере(ПараметрыСообщения);
	#Иначе
	Если ПараметрыСообщения.ОтправкаНаСервере Тогда
		// При работе в файловом режиме не нужно помещать файлы во временное хранилище,
		// а можно сразу отправить из клиентского приложения.
		ПодготовитьВложенияКОтправкеНаСервере(ПараметрыСообщения);
	Иначе
