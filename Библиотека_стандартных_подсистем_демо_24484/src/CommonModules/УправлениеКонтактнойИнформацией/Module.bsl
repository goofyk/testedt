...
Процедура УстановитьКонтактнуюИнформациюОбъектов(КонтактнаяИнформация, Замещать = Истина) Экспорт
	...
	Для каждого СтрокаКонтактнойИнформации Из КонтактнаяИнформация Цикл
		...
		ОбработатьСтрокуКонтактнойИнформации(СтрокаКонтактнойИнформации, ПараметрыКонтактнойИнформации.Периодическая);
	...
	Для каждого ВладелецКонтактнойИнформации Из ВладельцыКонтактнойИнформации Цикл
		...
		Для каждого КонтактнаяИнформацияОбъекта Из СтрокиКонтактнойИнформацииОбъекта Цикл
			Если Замещать Тогда
				Если ЗапрещенВводНесколькихЗначений(КонтактнаяИнформацияОбъекта.Вид, Объект.КонтактнаяИнформация) Тогда
					Продолжить;
				КонецЕсли;
				СтрокаКонтактнойИнформации = Объект.КонтактнаяИнформация.Добавить();
			Иначе
				СтрокаКонтактнойИнформации = Объект.КонтактнаяИнформация.Найти(КонтактнаяИнформацияОбъекта.ЗначенияПолей, "ЗначенияПолей");
				Если СтрокаКонтактнойИнформации = Неопределено Тогда
					Если ЗапрещенВводНесколькихЗначений(КонтактнаяИнформацияОбъекта.Вид, Объект.КонтактнаяИнформация) Тогда
						Продолжить;
					КонецЕсли;
			...
			ЗаполнитьКонтактнуюИнформациюОбъектаИзСтроки(КонтактнаяИнформацияОбъекта, ВладелецКонтактнойИнформации.Значение["Периодическая"], СтрокаКонтактнойИнформации);
		КонецЦикла;
...
Процедура УстановитьКонтактнуюИнформациюОбъекта(СсылкаИлиОбъект, Знач КонтактнаяИнформация, Замещать = Истина) Экспорт
	...
	Для каждого СтрокаКонтактнойИнформации Из КонтактнаяИнформация Цикл
		ОбработатьСтрокуКонтактнойИнформации(СтрокаКонтактнойИнформации, Периодическая);
	КонецЦикла;
	
	Если Замещать Тогда
		Если Периодическая Тогда
			Для каждого КонтактнаяИнформацияОбъекта Из КонтактнаяИнформация Цикл
				НайденныеСтроки = НайтиСтрокиКонтактнойИнформации(КонтактнаяИнформацияОбъекта.Вид, КонтактнаяИнформацияОбъекта.Дата, Объект.КонтактнаяИнформация);
	...
	Для каждого КонтактнаяИнформацияОбъекта Из КонтактнаяИнформация Цикл
		...
		Иначе
			СтрокаКонтактнойИнформации = Объект.КонтактнаяИнформация.Найти(КонтактнаяИнформацияОбъекта.ЗначенияПолей, "ЗначенияПолей");
			Если СтрокаКонтактнойИнформации = Неопределено Тогда
				Если ЗапрещенВводНесколькихЗначений(КонтактнаяИнформацияОбъекта.Вид, КонтактнаяИнформация) Тогда
					Продолжить;
				КонецЕсли;
				СтрокаКонтактнойИнформации = Объект.КонтактнаяИнформация.Добавить();
			КонецЕсли;
		...
		ЗаполнитьКонтактнуюИнформациюОбъектаИзСтроки(КонтактнаяИнформацияОбъекта, Периодическая, СтрокаКонтактнойИнформации);
	...
	Для Каждого ОбъектКонтактнойИнформации Из КонтактнаяИнформация Цикл
		...
		Иначе
			...
			Если ОбъектКонтактнойИнформации.ЭтоИсторическаяКонтактнаяИнформация Тогда
				ОбработатьСведенияКонтактнойИнформации(ОбъектКонтактнойИнформации, Форма);
				Продолжить;
			КонецЕсли;
...
Процедура ОбработатьСведенияКонтактнойИнформации(СтрокаКИ, Форма)
...
Процедура ЗаполнитьКонтактнуюИнформациюОбъектаИзСтроки(КонтактнаяИнформацияОбъекта, Периодическая, СтрокаКонтактнойИнформации)
	
	ЗаполнитьЗначенияСвойств(СтрокаКонтактнойИнформации, КонтактнаяИнформацияОбъекта);
	Если Периодическая Тогда
		СтрокаКонтактнойИнформации.ДействуетС = КонтактнаяИнформацияОбъекта.Дата;
	КонецЕсли;
	...
	ЗаполнитьТехническиеПоляКонтактнойИнформации(СтрокаКонтактнойИнформации, ОбъектКонтактнойИнформации, КонтактнаяИнформацияОбъекта.Тип);
	
КонецПроцедуры

Процедура ОбработатьСтрокуКонтактнойИнформации(СтрокаКонтактнойИнформации, Периодическая)
	...
	Если Периодическая
		И НЕ ЗначениеЗаполнено(СтрокаКонтактнойИнформации.Дата)
		И СтрокаКонтактнойИнформации.Вид.ХранитьИсториюИзменений Тогда
		СтрокаКонтактнойИнформации.Дата = ТекущаяДатаСеанса();
	КонецЕсли;
...
Процедура ОбновитьКонтекстноеМеню(Форма, ИмяЭлементаДляРазмещения)
	...
	Для Каждого СтрокаКИ Из ВсеСтроки Цикл
		Если СтрокаКИ.Тип <> Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			Продолжить;
		КонецЕсли;
		ЭлементФормы = Форма.Элементы.Найти("ПодменюКопироватьАдреса" + СтрокаКИ.ИмяРеквизита);
		ЭлементФормыКонтекстноеПодменю = Форма.Элементы.Найти("КонтекстноеПодменюКопироватьАдреса" + СтрокаКИ.ИмяРеквизита);
		Если ЭлементФормыКонтекстноеПодменю <> Неопределено Тогда
			...
			Для Каждого Адрес Из НайденныеСтроки Цикл
				...
				Если ЭлементФормы <> Неопределено Тогда
					...
					Если Кнопка = Неопределено Тогда
						Кнопка = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), ЭлементФормы);
						Команда = Форма.Команды.Добавить(ИмяКоманды);
						...
						ПараметрыКонтактнойИнформации.ДобавленныеЭлементы.Добавить(ИмяКоманды, 9, Истина);
					КонецЕсли;
					Кнопка.Заголовок = Адрес.Представление;
					Кнопка.Видимость = ЗначениеЗаполнено(Адрес.Представление);
				КонецЕсли;
				ИмяКоманды = "МенюПодменюАдрес" + СтрокаКИ.ИмяРеквизита + "_" + Адрес.ИмяРеквизита +"_";
				Кнопка = Форма.Элементы.Найти(ИмяКоманды);
				Если Кнопка = Неопределено Тогда
					Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), ЭлементФормыКонтекстноеПодменю);
					Кнопка.Заголовок = Адрес.Представление;
					Команда = Форма.Команды.Добавить(ИмяКоманды);
					Команда.Подсказка = НСтр("ru = 'Скопировать адрес'");
					Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
