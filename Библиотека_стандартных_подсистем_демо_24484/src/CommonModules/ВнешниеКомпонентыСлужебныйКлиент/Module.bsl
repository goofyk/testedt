...
Процедура ПодключитьКомпонентуИзРеестраWindowsЗавершение(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда 
		...
		Результат.Вставить("ОписаниеОшибки", НСтр("ru = 'Не удалось подключить внешнюю компоненту'"));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	...
	Попытка
		...
		Если ПодключаемыйМодуль = Неопределено Тогда
			ВызватьИсключение НСтр("ru = 'Не удалось создать экземпляр внешней компоненты'");
		КонецЕсли;
